...
exports.ecbDecryption = exports.ecbEncryption = exports.mix_columns = exports.keyExpansion = exports.zip = void 0;
...
function transpondString(str) {
    if (str.length !== 32) {
        throw new Error('Byte length is not 16 Byte!');
    }
    const array = [];
...
function transpondStringArray(arr) {
    const w0 = arr[0];
    const w1 = arr[1];
    const w2 = arr[2];
    const w3 = arr[3];
...
function wordIntoByteArray(word) {
    const byteArray = [];
...
function substitute(word, inverse) {
    const byteArray = [];
    for (let i = 0; i < word.length / HEXBYTES; i++) {
        const byte = word.slice(i * HEXBYTES, i * HEXBYTES + HEXBYTES);
...
function byteArrayIntoHex(word) {
    let hexString = '';
...
function keyExpansion(key) {
    ...
    const words = keyIntoByteArray(key);
    let round = 0;
    for (let i = 4; i < 44; i++) {
        ...
        const xorResult = (0, exports.zip)(xorValue1, xorValue2).map(([x, y]) => x ^ y);
        words.push(byteArrayIntoHex(xorResult));
    }
    return words.join('');
}
exports.keyExpansion = keyExpansion;
function addRoundKey(key, plaintext) {
    if (key.length !== ROUND_KEY_WORDS * WORD_BYTELENGTH * HEXBYTES) {
        throw new Error('Key length is not 16 Bytes!');
    }
    const xorArray = (0, exports.zip)(key.split(''), plaintext.split(''));
    const result = xorArray.map(([y, z]) => (parseInt(y, 16) ^ parseInt(z, 16)).toString(16));
    return result.join('');
}
...
function mix_columns(state, inverse) {
    const newState = [
        ['', '', '', ''],
        ['', '', '', ''],
        ['', '', '', ''],
    ...
    const matrixState = [];
    const transposedState = transpondString(state);
    for (let i = 0; i < 4; i++) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (!inverse) {
            ...
            newState[0][i] = (gmult(0x0e, s0) ^
                gmult(0x0b, s1) ^
                gmult(0x0d, s2) ^
                gmult(0x09, s3)).toString(16);
            newState[1][i] = (gmult(0x09, s0) ^
                gmult(0x0e, s1) ^
                gmult(0x0b, s2) ^
                gmult(0x0d, s3)).toString(16);
            newState[2][i] = (gmult(0x0d, s0) ^
                gmult(0x09, s1) ^
                gmult(0x0e, s2) ^
                gmult(0x0b, s3)).toString(16);
            newState[3][i] = (gmult(0x0b, s0) ^
                gmult(0x0d, s1) ^
                gmult(0x09, s2) ^
                gmult(0x0e, s3)).toString(16);
        }
    }
...
exports.mix_columns = mix_columns;
function reverseExpandedKey(key) {
...
function encrypt(plaintext, key) {
    ...
    if (plaintext.length !== SIZE_OF_AES_KEY_AND_STATE * HEXBYTES) {
        throw new Error('Plaintext length is not 16 Bytes!');
    }
    //expand key
    const expandedKey = keyExpansion(key);
    //add the first round key
    const firstRoundKey = expandedKey.slice(0, ROUND_KEY_WORDS * WORD_BYTELENGTH * HEXBYTES);
    ...
    for (let i = 0; i < NUMBER_OF_AES_ROUNDS; i++) {
        ...
        //add round key
        const roundKey = expandedKey.slice(firstRoundKey.length * (i + 1), firstRoundKey.length * (i + 1) + firstRoundKey.length);
...
function decrypt(ciphertext, key) {
    ...
    //add first round key
    let roundKey = expandedKey.slice(0, ROUND_KEY_WORDS * WORD_BYTELENGTH * HEXBYTES);
    ...
    for (let i = 0; i < NUMBER_OF_AES_ROUNDS; i++) {
        ...
        const substitutedStateStr = substitutedState.map(c => c.toString(16));
        ...
        //add round key
        roundKey = expandedKey.slice(roundKey.length * (i + 1), roundKey.length * (i + 1) + roundKey.length);
        tempState = addRoundKey(roundKey, substitutedStateStr.join(''));
        //last round, there will be no mixed columns
        if (i < NUMBER_OF_AES_ROUNDS - 1) {
            //mix columns
            tempState = transpondStringArray(transpondString(mix_columns(tempState, true)));
        }
    }
    return tempState;
}
function processInputText(hexString) {
    ...
    const paddingWord = hexString.slice(hexString.length - remindingChars);
    plaintexts.push(paddingWord.padEnd(32, '0'));
    return plaintexts;
}
const ecbEncryption = ({ plaintext, key }) => {
    if (key.length !== 32) {
        throw new Error('Key length is not 16 Bytes!');
    }
...
exports.ecbEncryption = ecbEncryption;
const ecbDecryption = ({ ciphertext, key }) => {
    if (key.length !== 32) {
        throw new Error('Key length is not 16 Bytes!');
    }
    if (ciphertext.length % 32 !== 0) {
        throw new Error('Ciphertext block length are not 16 Bytes!');
    }
    const ciphertexts = ciphertext.match(/.{32}/g) ?? [];
    const decryptedCiphers = [];
    for (let i = 0; i < ciphertexts.length; i++) {
        decryptedCiphers.push(decrypt(ciphertexts[i], key));
    }
    return (0, utils_1.hexToAscii)(decryptedCiphers.join(''));
};
exports.ecbDecryption = ecbDecryption;

...
function stringToArrayOfBits(str) {
    const arrayOfBytes = [];
    for (const c of str) {
...
function genDESKeys(key) {
    ...
    // Permuted Choice 2
    const keyBits_PC1_LCS_PC2 = keyBits_PC1_LCS.map(key => PC2.map(i => key[i - 1]));
...
function desRound(block, key) {
    ...
    // XOR with left block
    const block_E_XORkey_S_P_XOR_block_Left = block_E_XORkey_S_P.map((bit, i) => bit ^ block_Left[i]);
    return block_Right.concat(block_E_XORkey_S_P_XOR_block_Left);
}
function inversePermutation(block) {
    const IP_1 = [
...
const encrypt = ({ plaintext, key }) => {
    ...
    let parsedText = (0, utils_1.hexToAscii)(plaintext);
    ...
    return (0, utils_1.asciiToHex)(ciphertextArray.map(block => bitsToString(block)).join(''));
};
exports.encrypt = encrypt;
const decrypt = ({ ciphertext, key }) => {
    const parsedKey = (0, utils_1.hexToAscii)(key);
    let parsedText = (0, utils_1.hexToAscii)(ciphertext);

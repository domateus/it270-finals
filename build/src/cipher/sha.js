...
class Int64 {
    ...
    rotr(n) {
        ...
        if (n < 32) {
            tmp = 32 - n;
            return new Int64((this.high >>> n) | (this.low << tmp), (this.low >>> n) | (this.high << tmp));
        }
        else {
            tmp = 64 - n;
            return new Int64((this.low >>> n) | (this.high << tmp), (this.high >>> n) | (this.low << tmp));
        }
    }
    rotl(n) {
        let tmp;
    ...
    shr(n) {
        return new Int64(this.high >>> n, (this.low >>> n) | (this.high << (32 - n)));
    }
    static fromHex(hex) {
        if (hex.length > 16) {
    ...
    maj(x, y) {
        return new Int64((this.high & x.high) ^ (this.high & y.high) ^ (x.high & y.high), (this.low & x.low) ^ (this.low & y.low) ^ (x.low & y.low));
    }
    Sigma0() {
        const rotr28 = this.rotr(28);
        const rotr34 = this.rotr(34);
        const rotr39 = this.rotr(39);
        return new Int64(rotr28.high ^ rotr34.high ^ rotr39.high, rotr28.low ^ rotr34.low ^ rotr39.low);
    }
    Sigma1() {
        const rotr14 = this.rotr(14);
        const rotr18 = this.rotr(18);
        const rotr41 = this.rotr(41);
        return new Int64(rotr14.high ^ rotr18.high ^ rotr41.high, rotr14.low ^ rotr18.low ^ rotr41.low);
    }
    sigma0() {
        const rotr1 = this.rotr(1);
        const rotr8 = this.rotr(8);
        const shr7 = this.shr(7);
        return new Int64(rotr1.high ^ rotr8.high ^ shr7.high, rotr1.low ^ rotr8.low ^ shr7.low);
    }
    sigma1() {
        const rotr19 = this.rotr(19);
        const rotr61 = this.rotr(61);
        const shr6 = this.shr(6);
        return new Int64(rotr19.high ^ rotr61.high ^ shr6.high, rotr19.low ^ rotr61.low ^ shr6.low);
    }
    ch(x, y) {
        return new Int64((this.high & x.high) ^ (~this.high & y.high), (this.low & x.low) ^ (~this.low & y.low));
    }
...
const padding = (l) => {
    ...
    // but it does not suport the size that sha512 does
